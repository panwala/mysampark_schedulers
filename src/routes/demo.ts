...
const fontPath = path.join(__dirname, "../../public/font");
...
const generateImageBuffer = async (
  ...
  try {
    ...
    const scale = 2;
    const overlayCanvas = createCanvas(canvas.width * scale, canvas.height * scale);
    ...
    const calculatedWidth = desiredLogoHeight * aspectRatio;
    const logoXPosition = canvas.width / 2 - calculatedWidth / 2;
    ctx.drawImage(logoImage, logoXPosition, 50, calculatedWidth, desiredLogoHeight);
    ...
    for (const [index, row] of framesVariantLines.entries()) {
      ...
      ctx.font = `${fontWeight} ${fontSize}px ${fontFamily}`;
      const currentTextColor = textColours[index] === "Y" ? frameData.y : frameData.x;
      ...
      const totalTextWidth =
        index === 0
          ? texts.reduce(
            (sum: any, text: { text: any }) => sum + ctx.measureText(text.text).width,
            0
          )
          : texts.reduce(
            (sum: any, text: { text: any }) => sum + ctx.measureText(text.text).width,
      ...
      for (const text of texts) {
        ...
        const yPos = +row.y + +row.height / 2 + fontSize / 3 + bottomTextContentHeight;
        const textWidth = ctx.measureText(text.text).width;
